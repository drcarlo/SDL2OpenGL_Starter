...
void Texture::initFromFile(const std::string& file)
{
	...
	unsigned char* image = stbi_load(file.c_str(), &width, &height, &channels, STBI_rgb);
	...
	GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, (void*)image));
