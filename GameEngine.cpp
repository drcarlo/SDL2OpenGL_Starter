...
namespace GE {
	...
	bool GameEngine::init(bool vsync) {
		...
		cam = new Camera(glm::vec3(-5.0f, 0.0f, -5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 120.0f, (float)w / (float)h, 1.0f, 800.0f);
		cam->setTarget(glm::vec3(0.0f, 0.0f, 0.0f));
		modelRenderer = new ModelRenderer(cam);

		fontRenderer = new FontRenderer(w, h);
		fontRenderer->init();
		model = std::unique_ptr<Model>(new Model("resources/models/ship.obj"));
		model->init();
		...
		testUI = new ImgUi();
		testUI->init();
		testUI->setTexture(frameBuffer->getTextureLocation());
	...
	void GameEngine::update() {
		rotation += 0.2f;
		color.x = sinf(rotation * 0.1f);
		color.z = -sinf(rotation * - 0.1f);
		color.y = - sinf(rotation * 0.1f);
